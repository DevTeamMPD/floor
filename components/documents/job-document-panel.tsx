...
type Stage = { id: string; title: string; required: string[]; types: { value: string; label: string }[] };

const STAGES: Stage[] = [
  { id: "01-sales", title: "1. ข้อมูลขายและสำรวจ", required: ["สรุปงาน / ข้อมูลต้นทาง", "รูปสำรวจหรือเงื่อนไขหน้างาน"], types: [{ value: "job_summary", label: "สรุปงาน" }, { value: "site_survey", label: "แบบสำรวจ / รูปหน้างาน" }, { value: "external_reference", label: "เอกสารจากลูกค้า / BBPS" }] },
  { id: "02-planning", title: "2. แผนงาน", required: ["ใบสั่งงาน", "BOQ / รายการวัสดุ"], types: [{ value: "work_order", label: "ใบสั่งงาน" }, { value: "boq", label: "BOQ / รายการวัสดุ" }, { value: "technical_drawing", label: "แบบ / รายละเอียดเทคนิค" }] },
  { id: "03-warehouse", title: "3. คลังสินค้า", required: ["ใบหยิบสินค้า", "รูปสินค้าจริง"], types: [{ value: "pick_confirmation", label: "ใบยืนยันการหยิบ" }, { value: "picked_item_photo", label: "รูปสินค้าจริง" }] },
  { id: "04-installation", title: "4. ติดตั้ง", required: ["รูปก่อน–หลัง", "ลายเซ็นรับงาน", "รายงานเศษ"], types: [{ value: "installation_photo", label: "รูปก่อน–หลังติดตั้ง" }, { value: "customer_acceptance", label: "ลายเซ็น / ใบรับมอบ" }, { value: "remnant_report", label: "รายงานเศษ" }, { value: "installation_report", label: "รายงานติดตั้ง" }] },
  { id: "05-closing", title: "5. หลังการขายและปิดงาน", required: ["ใบส่งมอบ", "ประเมินความพึงพอใจ", "บันทึกติดตาม / NCR (หากมี)"], types: [{ value: "handover", label: "ใบส่งมอบ" }, { value: "csat", label: "ประเมินความพึงพอใจ" }, { value: "post_sales_followup", label: "บันทึกติดตามหลังการขาย" }, { value: "ncr", label: "NCR / ข้อร้องเรียน" }] },
];
...
export default function JobDocumentPanel({ jobNo }: { jobNo: string }) {
  ...
  async function upload() {
    ...
    if (file.size > 4 * 1024 * 1024) { toast.error("P1 รองรับไฟล์ไม่เกิน 4 MB; ไฟล์ใหญ่จะเพิ่มในรอบ upload session"); return; }
    ...
    setUploading(true);
    try {
      const response = await fetch("/api/job-documents", { method: "POST", body: form });
