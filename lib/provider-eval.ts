 ...
 *     วันที่ใบ NC ของบริษัทถูกเปิด คะแนนของทุกทีมต้องไม่กระโดดข้ามคืน ตัวคูณจึงไต่ทีละน้อย
 *     ถ้าใช้เกณฑ์แบบ "ครบ 5 ใบแล้วเปิดเต็ม" คะแนนทั้งบริษัทจะกระชากในคืนเดียว ซึ่งแย่พอกัน
 *
 *   คุณสมบัติที่ค่าคงที่ชุดนี้ถูกออกแบบมาให้ถือ (มีเทสยืนยันถึงใบที่ห้า):
 *     "ใบ NC หนึ่งใบที่เปิดในบริษัท ต้องไม่ทำให้ดาวของทีมที่ไม่ได้ถูกเปิด NC ใส่ ขยับเกิน
 *      0.05 ดวง และต้องไม่ทำให้ 'งานที่นับเป็นหลักฐาน NC ได้' ของทีมใดขยับเกิน 1 งาน"
 *
 *     ค่าจริงบนกองงานของบริษัทวันนี้ (2 ทีม 42 งาน คือ 23 + 19 ยังไม่มีใบ NC สักใบ)
 *     ใบที่   1      2      3      4      5
 *     ตัวคูณ  0.008  0.028  0.057  0.094  0.138
 *     (P4-9.2 เดิม: 0.043 แล้วกระโดดเป็น 0.159 ที่ใบที่สอง = เกือบสี่เท่าในคืนเดียว
 *      งานที่นับเป็นหลักฐานของทีม 19 งาน กระโดดจาก 0 เป็น 3 รวดเดียว ดันทีมที่ไม่ได้ทำอะไรเลย
 *      ข้าม MIN_DIRECT_EVIDENCE แล้วขึ้นดาว 3.68 ข้ามคืน — นั่นคือหน้าผาที่ถูกลบออกไป
 *      คอมเมนต์เดิมตรงนี้เขียนว่า "ใบแรกได้ตัวคูณราว 0.09" ซึ่งไม่ตรงกับกองงานจริงเลย
 *      0.09 คือค่าของพจน์ปริมาณอย่างเดียว ไม่ใช่ตัวคูณที่ใช้จริงซึ่งเป็นผลคูณสองพจน์)
 *
...
/**
 * P5-9 = P4-9.2 + ค่าคงที่ของตัวคูณความน่าเชื่อระบบ NC ที่ผูกกันด้วยสมการเดียวและไม่มีหน้าผา
 * แถวคะแนนที่เขียนไว้ก่อนหน้านี้ยังบันทึกว่าคำนวณด้วยวิธี P4-9.2 ตามความจริง ไม่ถูกเขียนทับ
 */
export const PROVIDER_EVAL_METHOD_VERSION = "P5-9";
 ...
 * ค่านี้เป็นข้อความเกี่ยวกับ "อุตสาหกรรม" ไม่ใช่เกี่ยวกับบริษัทนี้ จึงไม่ขยับตามปริมาณงาน
 */
export const NC_PROCESS_FULL_RATE = 0.05;

/**
 * จำนวนงานทั้งบริษัทที่ต้องมี "อัตรา" ข้างบนถึงจะเป็นการวัด ไม่ใช่เศษเสี้ยวของความบังเอิญ
 *
 * นี่คือค่าที่ทำให้ค่าคงที่สองตัวเลิกขัดกันเอง — ทั้งคู่ผูกกันด้วยสมการเดียว:
 *     NC_PROCESS_REPORTS_K = NC_PROCESS_FULL_RATE x NC_PROCESS_CREDIBLE_JOBS
 * อ่านเป็นภาษาคน: "ครึ่งทางของความน่าเชื่อ = บริษัทเปิด NC ที่อัตราเต็มมาแล้ว 200 งาน"
 * (ของเดิม K = 10 ใบ ยืนอยู่ข้าง ๆ FULL_RATE = 0.05 โดยไม่มีอะไรผูกกัน บริษัทที่เปิด NC
 *  ที่อัตราซึ่งสูตรเรียกว่า "เต็ม" พอดี กลับได้ความน่าเชื่อแค่ 0.167 และการจะไปถึง 0.5
 *  ต้องเปิด NC บน ~24% ของงาน = ห้าเท่าของอัตราที่สูตรเรียกว่าเต็ม)
 */
export const NC_PROCESS_CREDIBLE_JOBS = 200;

/** จำนวนใบ NC ทั้งบริษัทที่ทำให้ "ปริมาณการใช้ระบบ NC" ขึ้นไปครึ่งทาง (หน่วย: ใบ) */
export const NC_PROCESS_REPORTS_K = NC_PROCESS_FULL_RATE * NC_PROCESS_CREDIBLE_JOBS;
 ...
 *   ปริมาณ — "มีการใช้จริงหรือแค่มีคนลองกด" (หด: c / (c + K))
 *   อัตรา  — "ใช้ในระดับที่สมเหตุสมผลกับปริมาณงานหรือไม่"
 *            min(1, c / (FULL_RATE x (งานทั้งบริษัท + CREDIBLE_JOBS)))
 *
 * ทำไมพจน์ "อัตรา" ถึงบวก CREDIBLE_JOBS เข้าไปในฐาน:
 *   บริษัทนี้มีงานสะสมทั้งหมด 42 ใบ ถ้าคิดอัตราบน 42 ใบตรง ๆ "อัตราเต็ม" จะเท่ากับ NC เพียง
 *   2.1 ใบ พจน์นี้จึงกระโดดจาก 0.48 เป็น 0.95 ในการเปิด NC ใบที่สอง คูณกับพจน์ปริมาณที่
 *   โตพร้อมกัน = ความน่าเชื่อกระโดดเกือบสี่เท่าในคืนเดียว (0.043 -> 0.159) ซึ่งเป็นที่มาของ
 *   หน้าผาที่ใบที่สองพอดี การบวกฐานงานขั้นต่ำเข้าไปคือการพูดว่า "อัตราที่วัดจากงาน 42 ใบ
 *   ยังไม่ใช่อัตรา" — เป็นข้อเท็จจริงทางสถิติ ไม่ใช่การถ่วงให้ช้าไว้ก่อน
 */
export function ncrProcessCredibility(fleet: TeamEvalInput[]): number {
  ...
  // จำนวนใบที่ระบบซึ่งทำงานเต็มที่ "ควรจะออกได้" บนฐานงานที่บริษัททำมาแล้ว
  // บวกฐานงานขั้นต่ำที่ทำให้อัตราเป็นการวัดจริง — บริษัทที่เพิ่งทำงาน 42 ใบ ยังประเมินอัตราไม่ได้
  // พอ jobs โตเกิน NC_PROCESS_CREDIBLE_JOBS ไปมาก ๆ พจน์นี้ก็กลับไปเท่ากับ
  // min(1, อัตราจริง / NC_PROCESS_FULL_RATE) เหมือนเดิมทุกประการ
  const expectedAtFullRate = NC_PROCESS_FULL_RATE * (jobs + NC_PROCESS_CREDIBLE_JOBS);
  const rate = Math.min(1, reports / expectedAtFullRate);
