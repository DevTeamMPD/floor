...
function StarScore({ score }: { score: number | null }) {
  if (!score) return <span className="text-gray-300 text-sm">ยังไม่ให้คะแนน</span>;
  return (
        ...
        <div className="flex items-start justify-between px-6 py-4 border-b">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              {row.job_no}{row.customer ? ` — ${row.customer}` : ""}
            </p>
          </div>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
              ...
              {[1, 2, 3, 4, 5].map((s) => (
                <button
                  ...
                  className={`text-3xl transition-transform hover:scale-110 ${
                    s <= score ? "text-yellow-400" : "text-gray-200"
                  }`}
                >★</button>
              ))}
              {score > 0 && (
                <span className="ml-2 text-sm text-gray-500 self-center">{score}/5 ดาว</span>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <textarea value={issues} onChange={(e) => setIssues(e.target.value)}
              rows={3} placeholder="บันทึกปัญหาหรือข้อเสนอแนะ (ถ้ามี)"
...
function CsTrackingInner() {
  ...
  async function load() {
    const [{ data: jobs }, { data: evals }, { data: qs }] = await Promise.all([
      supabase
        .from("install_jobs")
        .select("job_no, customer, product, ticket, sku, closed_at, appt_date, phone")
        .eq("stage", 7)
    ...
    if (jobs) {
      ...
      setRows(
        jobs.map((j: Job) => ({ ...j, evaluation: evalMap.get(j.job_no) ?? null }))
      );
    }
        ...
        <div className="flex-1 overflow-auto">
          <table className="w-full text-sm">
            <thead className="bg-white border-b sticky top-0">
              <tr>
                {["เลขงาน", "ลูกค้า", "สินค้า", "นัดหมาย / ปิดงาน", "เบอร์โทร", "CS", "วันที่โทร", "คะแนน", "สถานะ", ""].map((h) => (
            ...
            <tbody className="divide-y divide-gray-100">
              {visible.map((row) => {
                ...
                const needsFollowup = !!ev?.needs_followup;
                return (
                  <tr key={row.job_no} className="hover:bg-blue-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-gray-800 max-w-[140px] truncate">{row.customer ?? "—"}</td>
                    <td className="px-4 py-3 text-gray-600 max-w-[160px] truncate">{row.product ?? "—"}</td>
                    <td className="px-4 py-3 text-gray-500 whitespace-nowrap">
                      {row.closed_at ? fmtDate(row.closed_at) : (row.appt_date ? fmtDate(row.appt_date) : "—")}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-1">
                        {evaluated ? (
                          ...
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-600 whitespace-nowrap">
                            🔴 รอโทร
                          </span>
                        )}
                        {needsFollowup && (
