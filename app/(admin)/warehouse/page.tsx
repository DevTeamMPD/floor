...
const EVIDENCE_EVENT_TYPES = new Set(["warehouse_completed", "warehouse_evidence_updated"]);
...
function photoUrl(supabase: ReturnType<typeof createClient>, path: string) {
  return path.startsWith("http") || path.startsWith("blob:") ? path : supabase.storage.from("job-photos").getPublicUrl(path).data.publicUrl;
}
...
export default function WarehouseWorkspacePage() {
  ...
  const [staff, setStaff] = useState<Record<string, Staff>>({}); const [items, setItems] = useState<WorkOrderItem[]>([]); const [events, setEvents] = useState<WorkOrderEvent[]>([]); const [loading, setLoading] = useState(true); const [saving, setSaving] = useState<string | null>(null); const [canAct, setCanAct] = useState(false); const [planOrderId, setPlanOrderId] = useState<string | null>(null);
  ...
  const [evidenceEditing, setEvidenceEditing] = useState(false); const [evidenceKeptPaths, setEvidenceKeptPaths] = useState<string[]>([]); const [evidenceNewFiles, setEvidenceNewFiles] = useState<WarehouseFilePreview[]>([]); const [evidenceNote, setEvidenceNote] = useState(""); const [evidenceSaving, setEvidenceSaving] = useState(false); const evidenceFilesRef = useRef<WarehouseFilePreview[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    const [jobResult, apptResult, staffResult, itemResult, eventResult, profileResult] = await Promise.all([
      ...
      rows.length ? supabase.from("floor_work_order_events").select("*").in("work_order_id", rows.map((row) => row.id)).in("event_type", Array.from(EVIDENCE_EVENT_TYPES)).order("occurred_at") : Promise.resolve({ data: [] }),
    ...
    setJobs(Object.fromEntries(((jobResult.data ?? []) as Job[]).map((row) => [row.job_no, row]))); setAppointments(Object.fromEntries(((apptResult.data ?? []) as Appointment[]).map((row) => [row.id, row]))); setStaff(Object.fromEntries(((staffResult.data ?? []) as Staff[]).map((row) => [row.id, row]))); setItems((itemResult.data ?? []) as WorkOrderItem[]); setEvents((eventResult.data ?? []) as WorkOrderEvent[]); setLoading(false);
  }, [supabase]);
  ...
  function openPlan(id: string) {
    ...
    setWarehouseNote(""); setEvidenceEditing(false); setPlanOrderId(id);
  }
  ...
  function startEvidenceEdit(currentPaths: string[]) { setEvidenceKeptPaths(currentPaths); setEvidenceNewFiles([]); setEvidenceNote(""); setEvidenceEditing(true); }
  ...
  async function saveEvidenceEdit() {
    ...
    if (isLocalDemo) {
      ...
      setEvidenceEditing(false); setEvidenceNewFiles([]); toast.success("แก้ไขรูปหลักฐานแล้ว (ข้อมูลจำลอง)"); return;
    }
    setEvidenceSaving(true); const uploaded: string[] = [];
    for (let index = 0; index < evidenceNewFiles.length; index++) {
  ...
  const planEvidenceEvent = planOrder ? events.filter((event) => event.work_order_id === planOrder.id).sort((a, b) => a.occurred_at.localeCompare(b.occurred_at)).at(-1) ?? null : null;
  const planEvidencePhotos = planEvidenceEvent?.photo_paths ?? [];
