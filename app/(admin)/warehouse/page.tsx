...
export default function WarehouseWorkspacePage() {
  ...
  const load = useCallback(async () => {
    ...
    setCanAct(["admin", "warehouse"].includes(String(profileResult.data?.role ?? "")));
  ...
  async function accept(id: string) { if (!canAct) { toast.error("บัญชีนี้ดูข้อมูลได้ แต่การรับงานเป็นหน้าที่ของคลัง"); return; } setSaving(id); const { error } = await supabase.rpc("accept_floor_warehouse_order_v2", { p_work_order_id: id }); setSaving(null); if (error) toast.error(error.message); else { toast.success("รับงานแล้ว"); void load(); } }
  ...
  return <div className="mx-auto max-w-7xl"><div><div className="text-xs font-semibold uppercase tracking-wider text-blue-600">คลังสินค้า</div><h1 className="mt-1 text-2xl font-bold text-slate-950">เตรียมสินค้าสำหรับติดตั้ง</h1><p className="mt-1 text-sm text-slate-500">รับงาน → ตรวจรายการ → บันทึกจำนวนจริงและรูป → ส่งไปรอติดตั้ง</p></div>
    {!canAct && !loading ? <div className="mt-4 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800">โหมดดูข้อมูล · เฉพาะคลังและผู้ดูแลระบบที่กดรับหรือปิดงานเตรียมสินค้าได้</div> : null}
