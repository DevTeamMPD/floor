...
import WarehouseLinePicking from "@/components/warehouse/line-picking";
import WarehouseOutstandingTools from "@/components/warehouse/outstanding-tools";
import { num, prefillActualQtyFromPicks, type WarehousePickLine } from "@/lib/warehouse-picking";
...
export default function WarehouseWorkspacePage() {
  ...
  function openPlan(id: string) {
    ...
    // P3-5: ถ้าคลังหยิบรายบรรทัดไปแล้ว ให้ใช้ตัวเลขนั้น จะได้ไม่ต้องพิมพ์ซ้ำตอนปิดงานทั้งใบ
    // บรรทัดที่ยังไม่ได้หยิบยังได้พฤติกรรมเดิมเป๊ะ ๆ (actual_qty ?? planned_qty)
    setActualQty(prefillActualQtyFromPicks(orderItems));
    setWarehouseNote(""); setPlanOrderId(id);
  }
  const applyPickedLines = useCallback((lines: WarehousePickLine[]) => {
    // แผงหยิบของเป็นแหล่งความจริงล่าสุดของ picked_qty จึงเอามา prefill ช่องของทางเดิมทั้งใบทุกครั้งที่รีเฟรช
    const rows = lines.flatMap((line) => line.item_id
      ? [{ id: line.item_id, planned_qty: num(line.planned_qty) ?? 0, actual_qty: num(line.actual_qty), picked_qty: num(line.picked_qty) }]
      : []);
    if (!rows.length) return;
    setActualQty((current) => ({ ...current, ...prefillActualQtyFromPicks(rows) }));
  }, []);
