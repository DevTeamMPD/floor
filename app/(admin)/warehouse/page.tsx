...
export default function WarehouseWorkspacePage() {
  ...
  const [staff, setStaff] = useState<Record<string, Staff>>({}); const [items, setItems] = useState<WorkOrderItem[]>([]); const [loading, setLoading] = useState(true); const [saving, setSaving] = useState<string | null>(null);
  const load = useCallback(async () => {
    ...
    const [jobResult, apptResult, staffResult, itemResult] = await Promise.all([
  ...
  async function accept(id: string) { setSaving(id); const { error } = await supabase.rpc("accept_floor_warehouse_order_v2", { p_work_order_id: id }); setSaving(null); if (error) toast.error(error.message); else { toast.success("รับงานแล้ว"); void load(); } }
