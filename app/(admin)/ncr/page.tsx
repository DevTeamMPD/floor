...
function JobCombobox({
  ...
  const filtered = query.length === 0
    ...
    : jobs
        .filter(
          (j) =>
            j.job_no.toLowerCase().includes(query.toLowerCase()) ||
            (j.customer ?? "").toLowerCase().includes(query.toLowerCase()) ||
            (j.ticket ?? "").toLowerCase().includes(query.toLowerCase())
        )
        .slice(0, 30);
  ...
  return (
    <div ref={ref} className="relative">
      <div className="flex gap-1">
        ...
        />
        {value && (
          <button onClick={clear} className="text-gray-400 hover:text-gray-600 px-2 text-lg">✕</button>
        )}
      </div>

      {value && (
        <p className="text-xs text-green-600 mt-0.5 pl-1">
          ✅ เลือกแล้ว: {jobs.find((j) => j.job_no === value)?.customer ?? value}
        </p>
      )}

      {open && filtered.length > 0 && (
        <div className="absolute z-50 left-0 right-0 mt-1 bg-white border rounded-xl shadow-lg max-h-52 overflow-y-auto">
          {filtered.map((j) => (
            <button
              key={j.job_no}
            ...
            >
              <span className="font-medium text-blue-700">{j.job_no}</span>
              {j.customer && <span className="text-gray-600 ml-2">{j.customer}</span>}
              {j.ticket && <span className="text-gray-400 ml-2 text-xs">#{j.ticket}</span>}
            </button>
          ))}
        </div>
      )}
      ...
      {open && query.length > 0 && filtered.length === 0 && (
        <div className="absolute z-50 left-0 right-0 mt-1 bg-white border rounded-xl shadow-lg px-3 py-3 text-sm text-gray-400">
...
function NcrPageInner() {
  ...
  useEffect(() => {
    async function init() {
      setLoading(true);
      const [{ data: jobData }, _] = await Promise.all([
        supabase
          .from("install_jobs")
          .select("job_no, customer, ticket")
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b bg-white shrink-0">
        <div>
          <h1 className="text-xl font-bold text-gray-900">🔴 NCR — Non-Conformance Report</h1>
          <p className="text-xs text-gray-500 mt-0.5">จัดการรายการเคลมสินค้าและข้อบกพร่อง</p>
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
            ...
            <div className="overflow-y-auto p-5 space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-sm font-medium text-gray-700">
                  เลือกงาน (Job No.) <span className="text-gray-400 font-normal">— เว้นไว้ถ้าไม่เกี่ยวข้องกับงานใด</span>
                </label>
                <JobCombobox
                  jobs={jobs}
                  value={form.job_no}
                  onChange={(v) => setForm({ ...form, job_no: v })}
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-sm font-medium text-gray-700">SKU สินค้า</label>
                  <input type="text" value={form.product_sku} onChange={(e) => setForm({ ...form, product_sku: e.target.value })}
                    placeholder="รหัสสินค้า" className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-400" />
                </div>
                <div className="space-y-1">
                  <label className="text-sm font-medium text-gray-700">จำนวน</label>
                  <input type="number" value={form.quantity} onChange={(e) => setForm({ ...form, quantity: e.target.value })}
                    placeholder="0" min="0" className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-400" />
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
                  rows={3} placeholder="อธิบายปัญหา สาเหตุ และผลกระทบ"
                  className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-400 resize-none" />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-sm font-medium text-gray-700">มูลค่าโดยประมาณ (฿)</label>
                  <input type="number" value={form.estimated_value_thb} onChange={(e) => setForm({ ...form, estimated_value_thb: e.target.value })}
                    placeholder="0.00" min="0" step="0.01" className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-400" />
                </div>
                <div className="space-y-1">
                  ...
                  <input type="text" value={form.created_by} onChange={(e) => setForm({ ...form, created_by: e.target.value })}
                    placeholder="ชื่อ-นามสกุล" className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-400" />
                </div>
              </div>
            </div>
            ...
            <div className="px-5 py-4 border-t flex gap-3">
              ...
              <button onClick={createNcr} disabled={saving}
            ...
            <div className="flex items-start justify-between px-5 py-4 border-b gap-3">
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  {selected.job_no ? `งาน: ${selected.job_no}` : "ไม่ระบุเลขงาน"} · {TYPE_LABELS[selected.type] ?? selected.type}
                </p>
              </div>
              <button onClick={() => setSelected(null)} className="text-gray-400 hover:text-gray-600 text-xl shrink-0">✕</button>
            </div>
            <div className="p-5 space-y-3 text-sm">
              {selected.product_sku && <div className="flex gap-2"><span className="text-gray-500 shrink-0">SKU:</span><span className="font-medium">{selected.product_sku}</span></div>}
              {selected.quantity != null && <div className="flex gap-2"><span className="text-gray-500 shrink-0">จำนวน:</span><span className="font-medium">{selected.quantity} ชิ้น</span></div>}
              {selected.estimated_value_thb != null && <div className="flex gap-2"><span className="text-gray-500 shrink-0">มูลค่า:</span><span className="font-medium">฿{selected.estimated_value_thb.toLocaleString()}</span></div>}
              {selected.description && <div><p className="text-gray-500 mb-1">รายละเอียด:</p><p className="text-gray-800 bg-gray-50 rounded-lg p-3 leading-relaxed">{selected.description}</p></div>}
              {selected.created_by && <div className="flex gap-2"><span className="text-gray-500 shrink-0">ผู้รายงาน:</span><span className="font-medium">{selected.created_by}</span></div>}
              <div className="flex gap-2"><span className="text-gray-500 shrink-0">วันที่:</span><span className="font-medium">{new Date(selected.created_at).toLocaleDateString("th-TH")}</span></div>
              {selected.closed_at && <div className="flex gap-2"><span className="text-gray-500 shrink-0">ปิด:</span><span className="font-medium">{new Date(selected.closed_at).toLocaleDateString("th-TH")}</span></div>}
            </div>
      ...
      {loading ? (
        ...
        <div className="flex-1 overflow-x-auto p-6">
          <div className="flex gap-4 min-h-[calc(100vh-120px)]">
            {STAGES.map((stage) => {
              const stageNcrs = filteredNcrs.filter((n) => n.status === stage.key);
              return (
                <div key={stage.key} className={`flex-1 min-w-[250px] rounded-xl border-2 ${stage.color} flex flex-col`}>
                  <div className="px-4 py-3 font-semibold text-sm flex items-center justify-between border-b border-current border-opacity-20">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${stage.badge}`}>{stageNcrs.length}</span>
                  </div>
                  <div className="flex-1 overflow-y-auto p-3 space-y-2">
                    {stageNcrs.length === 0 && <p className="text-xs text-gray-400 text-center py-6">ไม่มีรายการ</p>}
                    {stageNcrs.map((ncr) => (
                      <div key={ncr.id} onClick={() => setSelected(ncr)}
                        ...
                        <p className="text-sm font-medium text-gray-800 line-clamp-2">{ncr.title}</p>
                        <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                          <span className="text-xs text-gray-500">{TYPE_LABELS[ncr.type] ?? ncr.type}</span>
                          {ncr.job_no && <span className="text-xs text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded font-medium">{ncr.job_no}</span>}
                        </div>
                        {ncr.estimated_value_thb != null && <p className="text-xs text-gray-500 mt-1">฿{ncr.estimated_value_thb.toLocaleString()}</p>}
                        {ncr.product_sku && <p className="text-xs text-gray-400 mt-0.5">SKU: {ncr.product_sku}{ncr.quantity && ` × ${ncr.quantity}`}</p>}
                      </div>
                    ))}
                  </div>
                </div>
