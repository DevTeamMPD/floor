...
function CentralWorkOrderWorkspace({ jobNo, embedded = false, onChanged }: { jobNo: string; embedded?: boolean; onChanged?: () => void }) {
  ...
  const isAdmin = role === "admin"; const canAdminOverrideItems = isAdmin && order.status !== "head_review";
  const itemsUnlocked = canEdit || (canAdminOverrideItems && adminEditingItems);
    ...
    <div className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
      <div className="space-y-5">
        ...
        <Card title="3. วัสดุ อุปกรณ์ และของที่ต้องเตรียม" subtitle="หัวหน้ากำหนด SKU และจำนวนตามแผน คลังกรอกเฉพาะจำนวนหยิบจริง">
          {canAdminOverrideItems ? <div className="mb-4 rounded-xl border border-purple-200 bg-purple-50 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-sm font-semibold text-purple-950">🔓 แก้ไขรายการวัสดุ (สิทธิ์ผู้ดูแลระบบ)</div><p className="mt-1 text-xs text-purple-700">ใช้เมื่อพบว่ารายการวัสดุ/อุปกรณ์ไม่ครบหรือผิด แม้งานจะผ่านขั้นตอนหัวหน้าช่างไปแล้ว การแก้ไขจะถูกบันทึกในประวัติใบสั่งงาน</p></div>{adminEditingItems ? <div className="flex shrink-0 gap-2"><button type="button" onClick={cancelAdminEditItems} disabled={saving} className="rounded-lg border border-purple-300 bg-white px-3 py-2 text-xs font-semibold text-purple-700 disabled:opacity-50">ยกเลิก</button><button type="button" onClick={() => void saveAdminItems()} disabled={saving} className="rounded-lg bg-purple-600 px-3 py-2 text-xs font-semibold text-white disabled:opacity-50">{saving ? "กำลังบันทึก…" : "บันทึกการแก้ไข"}</button></div> : <button type="button" onClick={() => setAdminEditingItems(true)} className="shrink-0 rounded-lg bg-purple-600 px-3 py-2 text-xs font-semibold text-white">แก้ไขรายการ</button>}</div></div> : null}
          ...
          <div className="space-y-3">{items.map((item, index) => { const stock = materials.find((row) => row.sku === item.sku); const freeform = isFreeformNote(item); return <div key={item.id ?? index} className={`rounded-xl border p-4 ${freeform ? "border-violet-200 bg-violet-50/50" : item.category === "floor_material" && !item.sku ? "border-amber-300 bg-amber-50/50" : "border-slate-200"}`}>
            <div className="mb-3 flex items-center justify-between"><div className="font-medium text-slate-900">รายการที่ {index + 1}</div>{itemsUnlocked ? <button onClick={() => removeItem(index)} className="text-xs font-medium text-red-500">ลบรายการ</button> : null}</div>
            {freeform ? <div className="rounded-xl border border-violet-200 bg-white p-3"><div className="text-sm font-semibold text-violet-950">📝 โน้ต Freeform</div><p className="mt-1 text-xs text-violet-700">คำสั่งหรือข้อควรระวังถึงคลังและทีมช่าง · ไม่ใช่รายการเบิก จึงไม่ต้องระบุ SKU หรือจำนวน</p><label className="mt-3 block text-xs font-medium text-slate-600">ข้อความ *<textarea value={item.note} disabled={!itemsUnlocked} onChange={(e) => patchItem(index, { note: e.target.value })} rows={4} placeholder="เช่น นัดลูกค้าก่อนเข้าหน้างาน / ห้ามวางวัสดุในโถง / ต้องนำอุปกรณ์ป้องกันเพิ่ม" className="mt-1 w-full rounded-lg border bg-white px-3 py-2.5 text-sm disabled:bg-slate-100" /></label></div> : <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              <label className="text-xs font-medium text-slate-500">ประเภท *<select value={item.category} disabled={!itemsUnlocked} onChange={(e) => patchItem(index, { category: e.target.value as WorkItemCategory })} className="mt-1 w-full rounded-lg border bg-white px-3 py-2.5 text-sm">{WORK_ITEM_CATEGORIES.map((category) => <option key={category} value={category}>{WORK_ITEM_CATEGORY_LABELS[category]}</option>)}</select></label>
              <label className="text-xs font-medium text-slate-500">SKU {item.category === "floor_material" ? "*" : ""}{item.category === "floor_material" ? <select value={item.sku} disabled={!itemsUnlocked} onChange={(e) => selectMaterial(index, e.target.value)} className="mt-1 w-full rounded-lg border bg-white px-3 py-2.5 font-mono text-sm"><option value="">เลือก SKU</option>{materials.map((material) => <option key={material.id} value={material.sku}>{material.sku} · {material.name}</option>)}</select> : <input list="floor-material-skus" value={item.sku} disabled={!itemsUnlocked} onChange={(e) => selectMaterial(index, e.target.value)} placeholder="พิมพ์หรือเลือก SKU" className="mt-1 w-full rounded-lg border bg-white px-3 py-2.5 font-mono text-sm" />}{stock?.qty_on_hand != null ? <span className="mt-1 block text-[11px] text-emerald-600">คงเหลือ {Number(stock.qty_on_hand).toLocaleString()} {stock.unit || "หน่วย"}</span> : item.sku ? <span className="mt-1 block text-[11px] text-slate-500">รายการบริการจาก Catalog · คลังยังไม่ได้ระบุยอดคงเหลือ</span> : null}</label>
              <label className="text-xs font-medium text-slate-500">ชื่อสินค้า/อุปกรณ์ *<input value={item.itemName} disabled={!itemsUnlocked} onChange={(e) => patchItem(index, { itemName: e.target.value })} placeholder="ชื่อรายการ" className="mt-1 w-full rounded-lg border bg-white px-3 py-2.5 text-sm" /></label>
              <label className="text-xs font-medium text-slate-500">รุ่น สี ขนาด หรือล็อต<input value={item.specification} disabled={!itemsUnlocked} onChange={(e) => patchItem(index, { specification: e.target.value })} placeholder="เช่น Whitebuzz · 1.8 cm" className="mt-1 w-full rounded-lg border bg-white px-3 py-2.5 text-sm" /></label>
              <div className="grid grid-cols-[1fr_100px] gap-2"><label className="text-xs font-medium text-slate-500">จำนวนตามแผน *<input type="number" min="0" step="0.01" value={item.plannedQty} disabled={!itemsUnlocked} onChange={(e) => patchItem(index, { plannedQty: e.target.value })} className="mt-1 w-full rounded-lg border bg-white px-3 py-2.5 text-sm" /></label><label className="text-xs font-medium text-slate-500">หน่วย *<input value={item.unit} disabled={!itemsUnlocked} onChange={(e) => patchItem(index, { unit: e.target.value })} className="mt-1 w-full rounded-lg border bg-white px-3 py-2.5 text-sm" /></label></div>
              <label className="text-xs font-medium text-slate-500">แหล่งที่มา *<select value={item.sourceType} disabled={!itemsUnlocked} onChange={(e) => patchItem(index, { sourceType: e.target.value })} className="mt-1 w-full rounded-lg border bg-white px-3 py-2.5 text-sm"><option value="new">ของใหม่</option><option value="warehouse">ของในคลัง</option><option value="remnant">เศษวัสดุ</option><option value="bring_along">ช่างนำไป</option><option value="other">อื่น ๆ</option></select></label>
              <label className="text-xs font-medium text-slate-500 sm:col-span-2">หมายเหตุสำหรับคลัง/ช่าง<input value={item.note} disabled={!itemsUnlocked} onChange={(e) => patchItem(index, { note: e.target.value })} placeholder="ตำแหน่งใช้งาน วิธีแพ็ก หรือข้อควรระวัง" className="mt-1 w-full rounded-lg border bg-white px-3 py-2.5 text-sm" /></label>
          ...
          {itemsUnlocked ? <div className="mt-3 flex flex-wrap gap-2">{WORK_ITEM_CATEGORIES.map((category) => <button key={category} onClick={() => addItem(category)} className="rounded-lg border border-dashed border-blue-300 px-3 py-2 text-xs text-blue-700">+ {WORK_ITEM_CATEGORY_LABELS[category]}</button>)}<button onClick={addFreeformNote} className="rounded-lg border border-dashed border-violet-300 bg-violet-50 px-3 py-2 text-xs font-medium text-violet-700">+ โน้ต Freeform</button></div> : null}
        </Card>
      </div>
      ...
      <div className="space-y-5">
        ...
        <JobDocumentPanel jobNo={job.job_no} />
