...
export default function OperationsPage() {
  ...
  async function closeWork(workOrder: WorkOrder, customerName: string | null | undefined) {
    if (!canCloseWork) return;
    const reason = window.prompt(`ระบุเหตุผลที่สิ้นสุดงาน${customerName ? `: ${customerName}` : ""}`);
    if (!reason?.trim()) return;
    if (!window.confirm("ยืนยันสิ้นสุดงาน? งานจะถูกย้ายไปสถานะปิดงานและไม่แสดงในรายการดำเนินงานอีก")) return;
    const { error } = await supabase.rpc("close_floor_work_order_special", {
      p_work_order_id: workOrder.id,
      p_reason: reason.trim(),
    });
    if (error) {
      toast.error(`สิ้นสุดงานไม่สำเร็จ: ${error.message}`);
      return;
    }
  ...
  return <div className="mx-auto max-w-7xl"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><div className="text-xs font-semibold uppercase tracking-wider text-blue-600">หัวหน้าช่าง</div><h1 className="mt-1 text-2xl font-bold text-slate-950">ศูนย์ตัดสินใจและใบสั่งงาน</h1><p className="mt-1 text-sm text-slate-500">ตรวจข้อมูลเต็ม → จ่ายช่าง → สร้างรายการวัสดุ/อุปกรณ์ → ส่งให้คลัง</p></div><button onClick={() => void load()} className="rounded-xl border bg-white px-4 py-2 text-sm">รีเฟรช</button></div>
    ...
    {salesChat ? <div role="dialog" aria-modal="true" aria-label="ขอข้อมูลจากฝ่ายขาย" className="fixed inset-0 z-[70] flex items-end justify-center bg-slate-950/55 p-0 sm:items-center sm:p-4" onMouseDown={(event) => { if (event.target === event.currentTarget) setSalesChat(null); }}><div className="flex max-h-[92dvh] w-full max-w-2xl flex-col overflow-y-auto rounded-t-3xl bg-white p-4 shadow-2xl sm:rounded-3xl sm:p-5"><div className="mb-4 flex items-start justify-between gap-3"><div><h2 className="text-lg font-bold text-slate-950">ขอข้อมูลจากฝ่ายขาย</h2><p className="mt-1 text-sm text-slate-500">{salesChat.customerName || salesChat.jobNo} · ระบุสิ่งที่ต้องการในแชท แล้วส่งคำขอแก้ไข</p></div><button type="button" onClick={() => setSalesChat(null)} className="rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-600">ปิด ×</button></div><TicketChat jobNo={salesChat.jobNo} viewer="staff" viewerName="หัวหน้าช่าง" requestActionLabel="ส่งคำขอให้ฝ่ายขายแก้ไข" onRequestData={async (reason) => { await returnToSales(salesChat.workOrder.id, "direct", reason); setSalesChat(null); }} /></div></div> : null}
  </div>;
}
