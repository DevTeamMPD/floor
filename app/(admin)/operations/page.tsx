...
export default function OperationsPage() {
  ...
  const [loading, setLoading] = useState(true); const [loadError, setLoadError] = useState<string | null>(null); const [tab, setTab] = useState<"decision" | "returned" | "active">("decision"); const [canAct, setCanAct] = useState(false); const [canCloseWork, setCanCloseWork] = useState(false); const [expandedJobNo, setExpandedJobNo] = useState<string | null>(null); const [salesChat, setSalesChat] = useState<{ workOrder: WorkOrder; jobNo: string; customerName: string | null; source: string | null } | null>(null); const [showUnassignedWorkerAlert, setShowUnassignedWorkerAlert] = useState(false); const [closeWorkDialog, setCloseWorkDialog] = useState<{ workOrder: WorkOrder; customerName: string | null } | null>(null); const [closeReason, setCloseReason] = useState(""); const [closingWork, setClosingWork] = useState(false);
  // ด่านตรวจรับของงานที่กำลังจะสิ้นสุดทางพิเศษ
  //   undefined = ยังไม่โหลด · null = อ่านไม่ได้ (ถือว่ายังไม่ผ่าน ห้ามถือว่าผ่าน)
  // ต้องรู้ "ขาดข้อไหนบ้าง" ก่อนกดรับทราบ ไม่ใช่ถามลอย ๆ ว่า "แน่ใจหรือไม่"
  // เพราะการรับทราบที่ไม่รู้ว่ารับทราบอะไร ไม่ใช่การตัดสินใจ
  const [closeGate, setCloseGate] = useState<AcceptanceGate | null | undefined>(undefined);
  const [closeGateLoading, setCloseGateLoading] = useState(false);
  const [acknowledgeAcceptance, setAcknowledgeAcceptance] = useState(false);
  ...
  // อ่านด่านตรวจรับของงานที่กำลังจะสิ้นสุด เพื่อเอา "รายการที่ขาด" มาแสดงในกล่องยืนยัน
  const openCloseWorkDialog = useCallback(async (workOrder: WorkOrder, customerName: string | null) => {
    setCloseReason(""); setAcknowledgeAcceptance(false); setCloseGate(undefined); setCloseGateLoading(true);
    setCloseWorkDialog({ workOrder, customerName });
    const { data, error } = await supabase.rpc(ACCEPTANCE_GATE_RPC, {
      p_job_no: workOrder.job_no,
      p_appointment_id: workOrder.appointment_id,
    });
    setCloseGate(error ? null : parseAcceptanceGate(data));
    setCloseGateLoading(false);
  }, [supabase]);
  ...
  async function closeWork(workOrder: WorkOrder, reason: string) {
    ...
    if (error) {
      toast.error(`สิ้นสุดงานไม่สำเร็จ: ${floorErrorMessage(error)}`, { duration: 15000 });
      setClosingWork(false);
      return;
    }
    toast.success(acknowledgeAcceptance
      ? "สิ้นสุดงานแล้วทั้งที่เกณฑ์ตรวจรับยังไม่ครบ · รายการที่ขาดถูกบันทึกไว้ในประวัติใบสั่งงานแล้ว"
      : "สิ้นสุดงานแล้ว · ตรวจสอบย้อนหลังได้จากใบสั่งงาน");
    setCloseWorkDialog(null); setCloseReason(""); setAcknowledgeAcceptance(false); setCloseGate(undefined); setClosingWork(false);
    void load();
  }
  ...
  return <div className="mx-auto max-w-7xl"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><div className="text-xs font-semibold uppercase tracking-wider text-blue-600">หัวหน้าช่าง</div><h1 className="mt-1 text-2xl font-bold text-slate-950">ศูนย์ตัดสินใจและใบสั่งงาน</h1><p className="mt-1 text-sm text-slate-500">ตรวจข้อมูลเต็ม → จ่ายช่าง → สร้างรายการวัสดุ/อุปกรณ์ → ส่งให้คลัง</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => setShowUnassignedWorkerAlert(true)} className={`rounded-xl border px-4 py-2 text-sm font-semibold ${reviewedWithoutWorkers.length ? "border-amber-200 bg-amber-50 text-amber-800 hover:bg-amber-100" : "border-slate-200 bg-white text-slate-500"}`}>🔔 รอระบุผู้ทำงาน{reviewedWithoutWorkers.length ? ` (${reviewedWithoutWorkers.length})` : ""}</button><button onClick={() => void load()} className="rounded-xl border bg-white px-4 py-2 text-sm">รีเฟรช</button></div></div>
    ...
    {closeWorkDialog ? <div role="dialog" aria-modal="true" aria-label="ยืนยันสิ้นสุดงาน" className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/55 p-0 sm:items-center sm:p-5" onMouseDown={(event) => { if (event.target === event.currentTarget && !closingWork) { setCloseWorkDialog(null); setAcknowledgeAcceptance(false); setCloseGate(undefined); } }}><section className="flex max-h-[92dvh] w-full max-w-lg flex-col rounded-t-3xl bg-white shadow-2xl sm:rounded-3xl"><header className="border-b border-rose-200 bg-rose-50 px-5 py-4"><p className="text-xs font-semibold uppercase tracking-wide text-rose-700">Special action</p><h2 className="mt-1 text-lg font-bold text-rose-950">ยืนยันสิ้นสุดงาน</h2><p className="mt-1 text-sm text-rose-800">{closeWorkDialog.customerName || closeWorkDialog.workOrder.job_no} · งานจะถูกปิดและไม่แสดงในรายการดำเนินงานอีก</p></header><div className="max-h-[55dvh] overflow-y-auto p-5">
      {closeGateLoading ? <div className="mb-4 rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-xs text-slate-500">กำลังตรวจเกณฑ์ตรวจรับของงานนี้…</div> : null}
      {!closeGateLoading && closeGate !== undefined ? <div className={`mb-4 rounded-xl border p-3 text-xs leading-relaxed ${closeGate?.ok ? "border-emerald-200 bg-emerald-50 text-emerald-900" : "border-amber-300 bg-amber-50 text-amber-900"}`}>
        <div className="text-sm font-semibold">{closeGate?.ok ? "✅ " : "⛔ "}{gateHeadline(closeGate)}</div>
        {closeGate && !closeGate.ok ? <>
          <p className="mt-1 font-medium">รายการที่ยังขาดและจะถูกข้ามไปถ้ายืนยันสิ้นสุดงานตอนนี้:</p>
          <ul className="mt-1.5 list-disc space-y-0.5 pl-5">{closeGate.missing.map((miss) => <li key={`${miss.code}-${miss.reason}`}>{gateMissingLine(miss)}</li>)}</ul>
        </> : null}
        {closeGate === null ? <p className="mt-1">อ่านผลด่านตรวจรับไม่ได้ จึงยืนยันไม่ได้ว่างานนี้ผ่านเกณฑ์ — ถือว่ายังไม่ผ่านไว้ก่อน ถ้าจะสิ้นสุดงานต้องกดรับทราบด้านล่าง</p> : null}
        <details className="mt-2"><summary className="cursor-pointer font-medium">{ACCEPTANCE_RULE_HEADLINE}</summary>
          <ul className="mt-1 list-disc space-y-0.5 pl-5">{ACCEPTANCE_RULE_NOTICE_LINES.map((line) => <li key={line}>{line}</li>)}</ul>
        </details>
      </div> : null}
      <label className="text-sm font-semibold text-slate-800">เหตุผลที่สิ้นสุดงาน <span className="text-rose-600">*</span></label><textarea value={closeReason} onChange={(event) => setCloseReason(event.target.value)} placeholder="เช่น ลูกค้ายกเลิกงาน / งานไม่ต้องดำเนินการต่อ" rows={4} className="mt-2 w-full resize-none rounded-xl border border-slate-300 px-3 py-2 text-sm outline-none focus:border-rose-400 focus:ring-2 focus:ring-rose-100" autoFocus />
      {!closeGateLoading && closeGate !== undefined && !closeGate?.ok ? <label className="mt-4 flex items-start gap-2 rounded-xl border-2 border-rose-300 bg-rose-50 p-3 text-xs leading-relaxed text-rose-900">
        <input type="checkbox" checked={acknowledgeAcceptance} onChange={(event) => setAcknowledgeAcceptance(event.target.checked)} className="mt-0.5 h-4 w-4 shrink-0" />
        <span>ข้าพเจ้ารับทราบว่าเกณฑ์ตรวจรับ {closeGate ? `${closeGate.missing.length} รายการ` : "บางรายการ"} ข้างต้นยังไม่ครบ และยืนยันจะสิ้นสุดงานนี้ทั้งที่ยังขาด · ระบบจะบันทึกรายการที่ขาดพร้อมชื่อผู้กดไว้ในประวัติใบสั่งงาน</span>
      </label> : null}
    </div><footer className="flex justify-end gap-2 border-t border-slate-100 bg-slate-50 px-5 py-3"><button type="button" disabled={closingWork} onClick={() => { setCloseWorkDialog(null); setAcknowledgeAcceptance(false); setCloseGate(undefined); }} className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 disabled:opacity-50">ยกเลิก</button><button type="button" disabled={closingWork || closeGateLoading || closeGate === undefined || !closeReason.trim() || (!closeGate?.ok && !acknowledgeAcceptance)} onClick={() => void closeWork(closeWorkDialog.workOrder, closeReason)} className="rounded-lg bg-rose-600 px-4 py-2 text-sm font-semibold text-white hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-50">{closingWork ? "กำลังบันทึก…" : closeGateLoading || closeGate === undefined ? "กำลังตรวจเกณฑ์ตรวจรับ…" : closeGate?.ok ? "ยืนยันสิ้นสุดงาน" : acknowledgeAcceptance ? "ยืนยันสิ้นสุดงานทั้งที่เกณฑ์ยังไม่ครบ" : "ต้องกดรับทราบรายการที่ขาดก่อน"}</button></footer></section></div> : null}
