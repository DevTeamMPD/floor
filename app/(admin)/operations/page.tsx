...
export default function OperationsPage() {
  ...
  async function returnToSales(workOrderId: string, source: string | null | undefined) {
    ...
    if (error) toast.error(error.message); else { toast.success(source === "bbps" ? "ส่งกลับ BBPS แล้ว" : "ส่งกลับฝ่ายขายแล้ว"); void load(); }
  }
    ...
    <div className="mt-5 space-y-4">{loading ? <div className="rounded-2xl border bg-white p-10 text-center text-slate-400">กำลังโหลด…</div> : null}{!loading && !filtered.length ? <div className="rounded-2xl border bg-white p-10 text-center text-slate-400">ไม่มีงานในรายการนี้</div> : null}{filtered.map((appointment) => {
      ...
      return <article key={appointment.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b bg-slate-50/60 p-5"><div className="flex flex-col gap-3 sm:flex-row sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-lg font-semibold text-slate-950">{job?.customer_name || appointment.job_id || "งานติดตั้ง"}</h2><span className={`rounded px-2 py-0.5 text-[11px] ${job?.source === "bbps" ? "bg-orange-100 text-orange-700" : "bg-blue-100 text-blue-700"}`}>{job?.source === "bbps" ? "BBPS" : "ขายตรง"}</span></div><p className="mt-1 text-sm text-slate-500">{dateTime(appointment.slot_start)}–{new Date(appointment.slot_end).toLocaleTimeString("th-TH", { hour: "2-digit", minute: "2-digit", timeZone: "Asia/Bangkok" })} · {teams.find((team) => team.id === appointment.tech_id)?.name || "ยังไม่ระบุทีม"}</p><p className="mt-1 text-xs text-slate-400">#{job?.job_no || "—"}{job?.bill_no ? ` · บิล ${job.bill_no}` : ""}</p></div>{order ? <span className={`h-fit rounded-full px-3 py-1.5 text-xs font-medium ${workOrderStatusClass(order.status)}`}>{WORK_ORDER_STATUS_LABELS[order.status]}</span> : null}</div></div>
        <div className="grid gap-4 p-5 lg:grid-cols-[1.25fr_1fr]"><div className="space-y-3"><div className={`rounded-xl border p-4 ${readyData ? "border-emerald-200 bg-emerald-50/40" : "border-amber-200 bg-amber-50"}`}><div className="flex justify-between"><h3 className="font-semibold text-slate-900">ข้อมูลที่ต้องตัดสินใจ</h3><span className="text-xs">{readyData ? "✓ ข้อมูลหลักครบ" : "! ข้อมูลไม่ครบ"}</span></div><div className="mt-3 grid gap-2 text-sm sm:grid-cols-2"><div><span className="text-slate-400">โทร:</span> {job?.customer_phone || "—"}</div><div><span className="text-slate-400">สถานที่:</span> {job?.address || job?.location_url || "—"}</div><div><span className="text-slate-400">สินค้า:</span> {job?.product_name || appointment.requirement || "—"}</div><div><span className="text-slate-400">สำรวจ:</span> {survey || "ยังไม่มีรายละเอียด"}</div><div><span className="text-slate-400">รูปหน้างาน:</span> {job?.site_photos?.length || 0} รูป</div><div><span className="text-slate-400">หมายเหตุ:</span> {appointment.notes || "—"}</div></div></div>{orderItems.length ? <div className="rounded-xl border border-blue-100 bg-blue-50/50 p-4"><h3 className="text-sm font-semibold text-blue-950">วัสดุและอุปกรณ์ {orderItems.length} รายการ</h3><div className="mt-2 space-y-1 text-xs text-blue-800">{orderItems.slice(0, 5).map((item) => <div key={item.id}>{WORK_ITEM_CATEGORY_LABELS[item.category]} · {item.item_name} · {item.planned_qty} {item.unit}{item.actual_qty != null ? ` (หยิบจริง ${item.actual_qty})` : ""}</div>)}{orderItems.length > 5 ? <div>และอีก {orderItems.length - 5} รายการ</div> : null}</div></div> : null}</div>
          <div className="space-y-3"><div className="rounded-xl border p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h3 className="text-sm font-semibold">ช่างรายบุคคล</h3><p className="mt-1 text-xs text-slate-500">กำหนดหัวหน้าทีมก่อนยืนยัน จากนั้นส่งลิงก์ส่วนตัวให้แต่ละคนกด “รับทราบงาน”</p></div><TechnicianAssignmentButton appointmentId={appointment.id} appointmentTeamId={appointment.tech_id} jobNo={appointment.job_id} teams={teams} technicians={technicians} assignments={assignments} onChanged={() => void load()} /></div><div className="mt-3 space-y-2">{activeAssignments.map((assignment) => { const tech = technicians.find((row) => row.id === assignment.technician_id); return <div key={assignment.id} className="flex items-center justify-between rounded-lg bg-slate-50 p-2 text-xs"><span>{tech?.name || "ช่าง"}{assignment.is_lead ? " ★ หัวหน้า" : ""} · {assignment.acknowledged_at ? "รับทราบแล้ว" : assignment.first_opened_at ? "เปิดแล้ว" : "ยังไม่เปิด"}</span>{tech?.personal_token ? <button onClick={() => void copyLink(tech.personal_token)} className="rounded bg-blue-600 px-2 py-1 text-white">คัดลอกลิงก์รับทราบ</button> : null}</div>; })}{!activeAssignments.length ? <p className="text-xs text-amber-600">ยังไม่ได้จ่ายงานรายบุคคล</p> : null}</div></div>
            {tab === "decision" ? <div className="rounded-xl border-2 border-blue-200 bg-blue-50 p-4"><h3 className="font-semibold text-blue-950">การตัดสินใจของหัวหน้าช่าง</h3><p className="mt-1 text-xs text-blue-700">เลือกส่งกลับต้นทาง หรือเปิดรายการวัสดุเพื่ออนุมัติส่งคลัง</p><div className="mt-3 grid gap-2 sm:grid-cols-2"><button onClick={() => order && void returnToSales(order.id, job?.source)} className="rounded-xl border border-amber-300 bg-white px-3 py-3 text-sm font-semibold text-amber-700">↩ ส่งกลับ{job?.source === "bbps" ? " BBPS" : "ฝ่ายขาย"}</button><Link href={`/orders/${encodeURIComponent(job?.job_no || appointment.job_id || "")}`} className="rounded-xl bg-blue-600 px-3 py-3 text-center text-sm font-semibold text-white">{readyData && activeAssignments.some((row) => row.is_lead) ? "ตรวจวัสดุและอนุมัติ →" : "เปิดตรวจข้อมูลให้ครบ →"}</Link></div></div> : tab === "returned" ? <div className="rounded-xl border border-amber-300 bg-amber-50 p-4"><div className="font-semibold text-amber-900">รอ{job?.source === "bbps" ? " BBPS" : "ฝ่ายขาย"}แก้ไข</div><p className="mt-1 whitespace-pre-wrap text-sm text-amber-800">{order?.returned_reason || job?.flag_note || "ไม่ระบุเหตุผล"}</p></div> : <Link href={`/orders/${encodeURIComponent(job?.job_no || appointment.job_id || "")}`} className="block w-full rounded-xl bg-blue-600 py-3 text-center text-sm font-semibold text-white">เปิดใบสั่งงานฉบับเต็ม</Link>}
          </div></div>
      </article>;
    })}</div>
  </div>;
