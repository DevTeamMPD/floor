...
export default function RemnantsPage() {
  ...
  async function disposeRemnant(id: string) {
    ...
    if (error) toast.error(error.message); else { toast.success("ตัดจำหน่ายและบันทึกต้นทุนแล้ว"); fetch(); }
  }
  ...
  async function saveRate(matType: string) {
    ...
    if (error) toast.error(error.message); else { toast.success(`บันทึกต้นทุน ${matType} แล้ว`); fetch(); }
  }
