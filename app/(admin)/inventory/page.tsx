...
export default function InventoryPage() {
  ...
  async function saveMaterial() {
    if (!form.sku.trim() || !form.name.trim()) { toast.error('กรุณากรอก SKU และชื่อวัสดุ'); return; }
    setSaving(true);
  ...
  async function saveMovement(type: 'in' | 'out' | 'adjust') {
    ...
    setSaving(false);
    toast.success(type === 'in' ? 'รับสินค้าเข้าคลัง' : type === 'out' ? 'จ่ายสินค้าออกคลัง' : 'ปรับยอดสำเร็จ');
    setModalMode(null);
    load();
  }
    ...
    <div>
      ...
      <div className="flex items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">Stock on-hand · รับ/จ่าย/ปรับยอด</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-100 p-12 text-center text-slate-300">
          ...
          <div className="font-medium">{search || filterLow ? 'ไม่พบรายการที่ตรงเงื่อนไข' : 'ยังไม่มีวัสดุในคลัง — กด + เพิ่มวัสดุ'}</div>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setModalMode(null)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
            ...
            {(modalMode === 'receive' || modalMode === 'issue' || modalMode === 'adjust') && selectedMat && (
              <div className="p-6">
                <h2 className="text-lg font-semibold mb-1">
                  {modalMode === 'receive' ? '▲ รับสินค้าเข้าคลัง' : modalMode === 'issue' ? '▼ จ่ายสินค้าออก' : '⟳ ปรับยอดสต็อก'}
                </h2>
                <p className="text-sm text-slate-500 mb-4">{selectedMat.name} · คงเหลือ: <strong>{fmt(selectedMat.qty_on_hand)}</strong> {selectedMat.unit ?? ''}</p>
                <div className="space-y-3">
                  <div>
                    <label className="text-xs font-medium text-slate-500 mb-1 block">
                      {modalMode === 'adjust' ? 'ยอดใหม่ (ตั้งเป็น)' : 'จำนวน'} {selectedMat.unit ? `(${selectedMat.unit})` : ''} *
                    </label>
                  ...
                  {modalMode === 'issue' && (
                    <div>
                      <label className="text-xs font-medium text-slate-500 mb-1 block">เลขงาน (Job No.)</label>
            ...
            {modalMode === 'history' && selectedMat && (
              <div className="p-6">
                <h2 className="text-lg font-semibold mb-1">📋 ประวัติการเคลื่อนไหว</h2>
                <p className="text-sm text-slate-500 mb-4">{selectedMat.name}</p>
                {histLoading ? (
                  <div className="text-sm text-slate-400 animate-pulse py-4">โหลด…</div>
                ) : history.length === 0 ? (
                  <div className="text-sm text-slate-400 py-4 text-center">ยังไม่มีรายการ</div>
                ) : (
                  <div className="max-h-80 overflow-y-auto">
                    <table className="w-full text-xs">
                      <thead className="sticky top-0 bg-white">
                        <tr className="border-b border-slate-100">
                          {['วันที่', 'ประเภท', 'จำนวน', 'งาน', 'หมายเหตุ'].map((h) => (
                        ...
                        {history.map((mv) => {
                          ...
                          return (
                            <tr key={mv.id}>
                              ...
                              <td className="py-2 px-2 text-slate-400">{mv.ref_job_no ?? '—'}</td>
