-- FloorNow P3-7 (ฝั่งอ่านเท่านั้น): ทางอ่าน "รายการของที่ต้องเตรียมของงานหนึ่งใบ" ทางเดียว
--
-- ปัญหาเดิม: ผู้เรียกต้องรู้เองว่ารายการเตรียมของอยู่ได้ 3 ที่
--   1) floor_work_order_items  — ตารางจริงที่หน้าจอใช้ (แหล่งที่ถือว่าถูกต้อง)
--   2) floor_job_materials     — เข้าถึงผ่าน RPC เท่านั้น ไม่มีโค้ด TS อ้างถึงเลย (คนละ granularity: ตัวเลขแผ่นรวมต่อ 1 นัดหมาย)
--   3) install_jobs.pick_plan  — jsonb ยุคเดิม เขียนที่ components/pipeline/job-drawer.tsx เท่านั้น
-- ทำให้ทุกหน้าจอต้องเขียนตรรกะ fallback ซ้ำเอง และเสี่ยงเพี้ยนไปคนละแบบ
--
-- ไฟล์นี้รวม "ฝั่งอ่าน" ไว้ที่ฟังก์ชันเดียว โดยยังไม่แตะฝั่งเขียนของทั้ง 3 แหล่ง (นอกขอบเขตงานนี้โดยเจตนา)
-- floor_job_materials ไม่ถูกรวมเข้ามา เพราะเป็นตัวเลข "จำนวนแผ่นรวมทั้งใบ" ไม่ใช่รายบรรทัด
-- การยัดเข้ามาเป็นบรรทัดหนึ่งจะเป็นการแปลงความหมายข้อมูล ไม่ใช่การรวมทางอ่าน
--
-- ทำไมเลือก "ฟังก์ชัน" ไม่ใช่ "view":
--   * ต้องรับพารามิเตอร์ job_no และตัดสินใจ fallback แบบ "ต่อหนึ่งงาน" (มีบรรทัดจริงไหม)
--     view ทำได้แค่ผ่าน lateral/exists ทั้งตาราง ซึ่งแพงและอ่านยาก
--   * pick_plan ถูกเก็บเป็น "สตริงของ JSON" ในคอลัมน์ jsonb จึงต้องแกะสองชั้นและกันพัง
--     ซึ่งต้องใช้ exception handler — ทำใน view ไม่ได้
--   * ฟังก์ชันให้คุมสิทธิ์ได้ตรงกว่า: revoke จาก anon แล้ว grant execute ให้ authenticated อย่างเดียว
--     พร้อมด่านตรวจ is_floor_staff_active() ในตัว ซึ่งเทียบเท่า RLS ของ floor_work_order_items เดิม
--
-- อ่านอย่างเดียว ไม่เขียนอะไรทั้งสิ้น จึง stable และไม่ต้องเช็ค role ระดับ admin
...
comment on function public.get_job_prep_list(text) is
  'ทางอ่านรายการของที่ต้องเตรียมของงานหนึ่งใบ ทางเดียว: ใช้ floor_work_order_items เป็นหลัก '
  'และถอยไป install_jobs.pick_plan (ยุคเดิม) เฉพาะเมื่อยังไม่มีบรรทัดจริง โดยติดป้าย source ทุกแถวว่ามาจากไหน '
  'อ่านอย่างเดียว ไม่แตะฝั่งเขียนของแหล่งใดเลย';

revoke all on function public.get_job_prep_list(text) from public, anon;
grant execute on function public.get_job_prep_list(text) to authenticated;
