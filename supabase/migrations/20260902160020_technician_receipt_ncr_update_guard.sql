-- FloorNow (แก้ตามรีวิว D2 / D4 / D5): กฎ "บรรทัดไหนไม่ใช่ของ" ต้องมีที่เดียว
-- และผลตรวจรับของช่างต้องไม่หายเพราะ NC เขียนไม่ผ่าน
--
-- D2 — เดิม lib/technician-receipt.ts isNoteOnlyLine() อ้างว่าใช้ "เงื่อนไขเดียวกับ isFreeformWorkNote()"
--   แต่ตรวจแค่ 3 ใน 6 ข้อ (ขาด sourceType, sku, itemName) ผลคือของจริงถูกกรองหายจากหน้าจอช่าง:
--   บรรทัด category = 'tool' ที่บังเอิญ planned = 0 และหน่วยเป็น "รายการ" — ซึ่งเป็นเครื่องมือจริง
--   ที่หัวหน้าช่างพิมพ์เองแล้วยังไม่ระบุจำนวน — จะไม่มีปุ่มตรวจรับให้ช่างเลย และไม่มีใครรู้ว่าหายไปไหน
--   ตอนนี้ฝั่งหน้าจอใช้ lib/freeform-work-note.ts ตัวเดียวกันทุกที่ ซึ่งต้องรู้ค่า source_type
--   ฟังก์ชันอ่านสองตัวนี้จึงต้องส่ง sourceType กลับไปด้วย มิฉะนั้นกฎครบ 6 ข้อจะตัดสินไม่ได้
--
-- D5 — หน้าคลัง (components/warehouse/line-picking.tsx) ไม่ได้กรองบรรทัดโน้ตของหัวหน้าช่างเลย
--   คนคลังจึงเห็นปุ่ม "หยิบครบ / บางส่วน / ไม่มีของ" บนบรรทัดที่ไม่มีของให้หยิบ (โปรดักชันมีจริง 3 บรรทัด)
--   get_warehouse_pick_lines จึงต้องคืน source_type ด้วย เพื่อให้ฝั่งคลังใช้กฎตัวเดียวกันได้
--   ฟังก์ชันนี้คืน TABLE จึงเปลี่ยนคอลัมน์ด้วย create or replace ไม่ได้ ต้อง drop แล้วสร้างใหม่
--   (เป็นฟังก์ชันที่สาขานี้เพิ่งสร้างเอง ไม่ใช่ของเดิม และ grant ถูกตั้งคืนครบในไฟล์นี้)
--
-- D4 — record_technician_item_receipt: ทาง "เปิด NC ใบใหม่" มี exception handler ครอบไว้
--   พร้อมคอมเมนต์ว่า "ผลตรวจรับของช่างสำคัญกว่าการเปิด NC สำเร็จ" แต่ทาง "อัปเดต NC ใบเดิม"
--   ไม่มี handler เลย ถ้า update ncr_reports พลาด ทั้งธุรกรรม rollback และสิ่งที่ช่างเพิ่งกรอกหายทั้งก้อน
--   ซึ่งขัดกับเจตนาที่เขียนไว้เอง ตอนนี้ทั้งสองทาง (และทาง "แก้กลับเป็นได้ครบ") ทำเหมือนกันหมด
--   ต่างกันแค่: ทางเปิดใหม่ตั้ง v_ncr_id = null เพราะใบนั้นไม่เคยถูกสร้าง
--   ส่วนทางอัปเดตคง v_ncr_id ไว้ เพราะใบเดิมยังอยู่จริง แค่อัปเดตไม่ผ่าน — ทั้งสองรายงานผ่าน ncrError
--   พิสูจน์ด้วยโพรบ D4-a / D4-b ที่รันจริง ดู sdd-jobtpl/p35fix-probes.sql
--
-- additive ล้วน: แทนที่ฟังก์ชันของสาขานี้เอง ไม่แตะโครงตาราง ไม่แตะข้อมูลแถวใด
